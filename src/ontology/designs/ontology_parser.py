...
from designs.rule_parser import RuleCreator
...
class OntologyParser: 
                ...
                with StepContext(name="Batching Ontology Inference", catch=(RuntimeError,)):
                    ...
                    if need_sync: 
                        ...
                        with StepContext(name="Crate Label", catch=(RuntimeError,)):
                            self.rule_parser.create_labels(actor, filepath, len(batch_states), index=index)
    ...
    def print_results(self): 
        print(f"Reason times (s): {self.ev.metrics.reason_times}")
        print(f"Throughput (states/s): {self.ev.metrics.throughput}")
        print(f"Memory (KB): {self.ev.metrics.mem_snapshots[-1] if self.ev.metrics.mem_snapshots else None}")
        print(f"Undefined ratios: {self.ev.metrics.undefined_ratios[-1] if self.ev.metrics.undefined_ratios else None}")
        print(f"Functional violations (last): {self.ev.metrics.functional_violations[-1] if self.ev.metrics.functional_violations else None}")
        print(f"Label distributions: {self.ev.metrics.distributions}")
        crostab = defaultdict()
        for k, v in self.ev.metrics.crosstabs.items():
            crostab[k] = list(v.items())[:5]
        print(f"Crosstabs sample: {crostab}")
